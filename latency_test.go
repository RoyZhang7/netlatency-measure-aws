...
func main() {
    ...
    if *mode == "client" {
        ...
        for _, p := range bytes.Split(*protocolsFlag, []byte(",")) {
            protocols = append(protocols, string(bytes.TrimSpace(p)))
        }
        ...
        if err := tcpServer.Start(); err != nil {
            fmt.Println("Failed to start TCP server: %v\n", err)
            return
        }

        httpServer := NewHTTPServer(*httpPort)
        if err := httpServer.Start(); err != nil {
            fmt.Println("Failed to start HTTP server: %v\n", err)
            return
        }
